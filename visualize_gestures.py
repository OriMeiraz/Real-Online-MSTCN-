...
def _draw_prediction_bars(predictions, ground_truth, actions_dict):
    ...
    fig, axs = plt.subplots(len(gestures_list), 1,
                            figsize=(14,  0.7*len(gestures_list)))
    ...
    for i, gestures in enumerate(gestures_list):
        ...
        axs[i].imshow(map, cmap=cmap_, aspect="auto",
...
def draw_prediction_bars_on_frame(frame, predictions, ground_truth, actions_dict):
    ...
    pil_frame.paste(prediction_bar, (int(0.15*width),
                    height-int(0.1*height)), prediction_bar)
...
def process_video(video_path, results, gt_results, output_root, gesture_to_name, actions_dict):
    ...
    while (cap.isOpened()):
        ...
        img = draw_prediction_bars_on_frame(
            img, predictions, ground_truth, actions_dict)
