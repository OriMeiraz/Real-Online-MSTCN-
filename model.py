...
class MST_TCN2_late(nn.Module):
    def __init__(self, num_layers_PG, num_layers_R, num_R, num_f_maps, dim, num_classes_list, dropout=0.5, window_dim=0, offline_mode=False):
        ...
        if num_R > 0:  # num_R = number of refinements layers
    ...
    def forward(self, x, *args):
        ...
        if self.num_R > 0:
            for k, R in enumerate(self.Rs):  # R = refinement layer
                if k != len(self.Rs):  # if not last layer - always
                    outs = R(F.softmax(out, dim=1), 0, self.offline_mode)
                else:
                    print("weird")
                    outs = R(F.softmax(out, dim=1),
                             self.window_dim, self.offline_mode)
    ...
    def forward(self, x, window_dim, offline_mod):
        ...
        for i in range(self.num_layers):
            ...
            f = self.conv_fusion[i](torch.cat([self.conv_dilated_1[i](
                f, window_dim, offline_mod), self.conv_dilated_2[i](f, window_dim, offline_mod)], 1))
            f = F.relu(f)
            f = self.dropout(f)
            f = f + f_in
...
class Dilated_conv(nn.Module):
    ...
    def window_padding(self, input, padding_dim, window_dim=0):
        ...
        padding_left = torch.zeros(
            input.shape[0], input.shape[1], 2*padding_dim - window_dim).to(input.device)
        padding_right = torch.zeros(
            input.shape[0], input.shape[1], window_dim).to(input.device)
