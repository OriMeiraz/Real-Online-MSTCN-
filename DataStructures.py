...
class ModelRecreate:
    def __init__(self, model, w_max, frame_gen, extractor, *args) -> None:
...
class PgStage:
    def __init__(self, model, w_max, frame_gen, extractor, *args) -> None:
        self.val_augmentation = transforms.Compose([GroupScale(int(256)),
                                                    GroupCenterCrop(224)])
        self.normalize = GroupNormalize(args[0], args[1])
        ...
        self.top_layer = PredictionLayer(
            model, model.PG.num_layers-1, w_max, self, batch_size=frame_gen.batch_size)
...
class RefinementStage:
    def __init__(self, model, w_max, stage_num, frame_gen, extractor, *args) -> None:
        if stage_num != 0:
            self.prev_stage = RefinementStage(
                model, w_max, stage_num-1, frame_gen, extractor, *args)
        else:
            self.prev_stage = PgStage(
                model, w_max, frame_gen, extractor, *args)
        ...
        self.top_layer = RefinementLayer(
            model, len(model.Rs[0].layers)-1, w_max, stage_num, self, batch_size=frame_gen.batch_size)
