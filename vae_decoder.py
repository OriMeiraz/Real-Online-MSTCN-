...
class GreenBlock(nn.Module):
    def __init__(self, in_chnnels):
        ...
        self.conv1 = nn.Conv2d(in_chnnels, in_chnnels, kernel_size=(1,1), padding="same")
        self.group_norm1 = nn.GroupNorm(num_groups=8, num_channels=in_chnnels)
        self.conv2 = nn.Conv2d(in_chnnels, in_chnnels, kernel_size=(3, 3), padding="same")
...
class VAEDecoder(nn.Module):
    def __init__(self, intermediate_image_shape=(16, 14, 14), input_size=2048,
        ...
        if isinstance(intermediate_image_shape, int):
            intermediate_image_shape = (16, intermediate_image_shape // 16, intermediate_image_shape // 16)
        ...
        self.linear_layer = nn.Linear(intermediate_size, self.base_dim * self.base_height * self.base_width)

        self.conv1 = nn.Conv2d(self.base_dim, base_fillter_num, kernel_size=(1,1))
        self.up_samapling1 = nn.ConvTranspose2d(base_fillter_num, base_fillter_num, kernel_size=2, stride=2)
        ...
        self.conv_ending = nn.Conv2d(base_fillter_num // 8, output_channel_num, kernel_size=(1,1))
