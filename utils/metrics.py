...
class ComputeMetrics:
    ...
    def __init__(self, metric_types=None, overlap=.1, bg_class=None, n_classes=None):
        ...
        self.attrs = {"overlap": overlap,
                      "bg_class": bg_class, "n_classes": n_classes}
        self.trials = []
    ...
    def print_scores(self, metric_types=None):
        ...
        scores = [np.mean([self.scores[m][trial]
                          for trial in self.trials]) for m in metric_types]
        txt = "All: " + " ".join(["{}:{:.04}".format(metric_types[i], scores[i])
...
def macro_accuracy(P, Y, n_classes, bg_class=None, return_all=False, **kwargs):
    def macro_(P, Y, n_classes=None, bg_class=None, return_all=False):
        ...
        if bg_class is not None:
            diag = np.array([diag[i]
                            for i in range(n_classes) if i != bg_class])
    ...
    if type(P) == list:
        out = [macro_(P[i], Y[i], n_classes=n_classes, bg_class=bg_class,
                      return_all=return_all) for i in range(len(P))]
        if return_all:
...
def classification_accuracy(P, Y, bg_class=None, **kwargs):
    ...
    def clf_(p, y, bg_class):
        ...
        for i in range(len(S_true)):
            ...
            if p.ndim == 1:
                pred_label = scipy.stats.mode(
                    p[I_true[i][0]:I_true[i][1]])[0][0]
            else:
        ...
        for i in range(n_true_segs):
            for j in range(n_pred_segs):
                if true_labels[i] == pred_labels[j]:
                    intersection = min(pred_intervals[j][1], true_intervals[i][1]) - max(
                        pred_intervals[j][0], true_intervals[i][0])
                    union = max(pred_intervals[j][1], true_intervals[i][1]) - \
...
def segment_data(Xi, Yi):
    idxs = [0] + (np.nonzero(np.diff(Yi))[0]+1).tolist() + [len(Yi)]
    Xi_split = [np.squeeze(Xi[:, idxs[i]:idxs[i+1]])
