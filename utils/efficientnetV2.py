    ...
    def __init__(self, size, num_classes, pretrained, input_size=224):
        ...
        if size == "s":
            self.base = timm.create_model(
                "tf_efficientnetv2_s", pretrained=pretrained, num_classes=1)
        elif size == "m":
            self.base = timm.create_model(
                "tf_efficientnetv2_m", pretrained=pretrained, num_classes=1)
        elif size == "l":
            self.base = timm.create_model(
                "tf_efficientnetv2_l", pretrained=pretrained, num_classes=1)
        else:
